...
class Pais extends CI_Controller {
    ...
    public function nuevo_pais(){        
        echo json_encode($this->Pais_model->insert_pais());         
    }
    
    public function modificar_pais(){
        echo json_encode($this->Pais_model->edit_pais());         
    }
    ...
    public function tabla_principal_paises($count = 5, $page = 1, $order = 'nombre', $type = 'asc'){
        ...
        $array = $this->Pais_model->generar_json_tabla_principal($inicio, $count, $order, $type);
    ...
    public function ver ($id=1){             
        $result=$this->Pais_model->get_pais($id);
        foreach ($result as $row) {
    ...
    public function ver_sel($activos=true){        
        $res=array();
        $result = $this->Pais_model->get_pais_sel($activos);
        foreach ($result as $row) {
            $res[]= $row;            
        }
    ...
    public function index() {
        ...
        $this->load->view('Admin/Pais', $data);
