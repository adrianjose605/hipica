...
class Entrenadores_model extends CI_Model {
...
//put your code here
    ...
    public function search_entrenadores($descripcion = '') {
        $this->db->select("CONCAT(primer_nombre,' ',segundo_nombre,' ',primer_apellido,' ',segundo_nombre) AS entrenador ,identrenador AS id",false);
        $this->db->from('tbpersona');
        $this->db->join('tbentrenador', 'tbpersona.idpersona=tbentrenador.idpersona');
        $this->db->where('tbentrenador.status','1');
    ...
    public function generar_json_tabla_principal($offset, $cantidad, $order, $type) {
        ...
        $query1 = $this->db->get('tbentrenador_detallado');
        ...
        $query = $this->db->get('tbentrenador_detallado');
    ...
    public function edit_pais() {
        ...
        if ($this->db->update('tbentrenador_detallado', $arr)) {
            $res['status'] = 1;
