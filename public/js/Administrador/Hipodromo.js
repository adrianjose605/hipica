...
controller('hipodromo', ['$scope','$http','$mdToast', function($scope, $http,$mdToast){
	$scope.obj={};
	$scope.obj2={};
	$scope.spaises=[];	
	...
	$scope.crear_nuevo= function(tipo){
		...
		if(tipo){
			url='Admin/hipodromo/crear_hipodromo';
			obj=$scope.obj;
		}else{
			url='Admin/hipodromo/modificar_hipodromo';
			obj=$scope.obj2;
		}
	...
	$scope.get= function(id){	
		$scope.cargarTipos();
		$http.get('Admin/hipodromo/ver/'+id).
		success(function(data, status, headers, config) {				
	...
	$scope.getResource = function (params, paramsObj) {	
		...
		var urlApi = 'Admin/hipodromo/tabla_principal_hipodromo/'+paramsObj.count+'/'+paramsObj.page+'/';
		if(paramsObj.sortBy){
