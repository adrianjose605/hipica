...
controller('tipounidad', ['$scope','$http','$mdToast', function($scope, $http,$mdToast){
	$scope.obj={};
	$scope.obj2={};
	...
	$scope.crear_nuevo= function(tipo){
		...
		if(tipo){
			url='Admin/tipoUnidad/crear_tipo_unidad';
			obj=$scope.obj;
		}else{
			url='Admin/tipoUnidad/modificar_tipo_unidad';
			obj=$scope.obj2;
		}
		console.log(url);
		console.log(obj);
	...
	$scope.get= function(id){		
		$http.get('Admin/tipoUnidad/ver/'+id).
		success(function(data, status, headers, config) {				
	...
	$scope.getResource = function (params, paramsObj) {	
		...
		var urlApi = 'Admin/tipoUnidad/tabla_principal_tipo_unidad/'+paramsObj.count+'/'+paramsObj.page+'/';
		if(paramsObj.sortBy){
