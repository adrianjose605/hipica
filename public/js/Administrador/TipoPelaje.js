...
controller('tipopelaje', ['$scope','$http','$mdToast', function($scope, $http,$mdToast){
	$scope.obj={};
	$scope.obj2={};
	...
	$scope.crear_nuevo= function(tipo){
		...
		if(tipo){
			url='Admin/tipoPelaje/crear_tipo_pelaje';
			obj=$scope.obj;
		}else{
			url='Admin/tipoPelaje/modificar_tipo_pelaje';
			obj=$scope.obj2;
		}
	...
	$scope.get= function(id){		
		$http.get('Admin/tipoPelaje/ver/'+id).
		success(function(data, status, headers, config) {				
	...
	$scope.getResource = function (params, paramsObj) {	
		...
		var urlApi = 'Admin/tipoPelaje/tabla_principal_tipo_pelaje/'+paramsObj.count+'/'+paramsObj.page+'/';
		if(paramsObj.sortBy){
