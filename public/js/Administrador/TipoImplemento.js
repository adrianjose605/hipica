...
controller('tipoimplemento', ['$scope','$http','$mdToast', function($scope, $http,$mdToast){
	$scope.obj={};
	$scope.obj2={};
	...
	$scope.crear_nuevo= function(tipo){
		...
		if(tipo){
			url='Admin/tipoImplemento/crear_tipo_implemento';
			obj=$scope.obj;
		}else{
			url='Admin/tipoImplemento/modificar_tipo_implemento';
			obj=$scope.obj2;
		}
	...
	$scope.get= function(id){		
		$http.get('Admin/tipoImplemento/ver/'+id).
		success(function(data, status, headers, config) {				
	...
	$scope.getResource = function (params, paramsObj) {	
		...
		var urlApi = 'Admin/tipoImplemento/tabla_principal_tipo_implemento/'+paramsObj.count+'/'+paramsObj.page+'/';
		if(paramsObj.sortBy){
