...
controller('tipoorigen', ['$scope','$http','$mdToast', function($scope, $http,$mdToast){
	$scope.obj={};
	$scope.obj2={};
	...
	$scope.crear_nuevo= function(tipo){
		...
		if(tipo){
			url='Admin/tipoOrigen/crear_tipo_origen';
			obj=$scope.obj;
		}else{
			url='Admin/tipoOrigen/modificar_tipo_origen';
			obj=$scope.obj2;
		}
		console.log(url);
		console.log(obj);
	...
	$scope.get= function(id){		
		$http.get('Admin/tipoOrigen/ver/'+id).
		success(function(data, status, headers, config) {				
	...
	$scope.getResource = function (params, paramsObj) {	
		...
		var urlApi = 'Admin/tipoOrigen/tabla_principal_tipo_origen/'+paramsObj.count+'/'+paramsObj.page+'/';
		if(paramsObj.sortBy){
