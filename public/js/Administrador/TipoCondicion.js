...
controller('tipocondicion', ['$scope','$http','$mdToast', function($scope, $http,$mdToast){
	$scope.obj={};
	$scope.obj2={};
	...
	$scope.crear_nuevo= function(tipo){
		...
		if(tipo){
			url='Admin/tipoCondicion/crear_tipo_condicion';
			obj=$scope.obj;
		}else{
			url='Admin/tipoCondicion/modificar_tipo_condicion';
			obj=$scope.obj2;
		}
	...
	$scope.get= function(id){		
		$http.get('Admin/tipoCondicion/ver/'+id).
		success(function(data, status, headers, config) {				
	...
	$scope.getResource = function (params, paramsObj) {	
		...
		var urlApi = 'Admin/tipoCondicion/tabla_principal_tipo_condicion/'+paramsObj.count+'/'+paramsObj.page+'/';
		if(paramsObj.sortBy){
