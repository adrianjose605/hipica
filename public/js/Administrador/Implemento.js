...
controller('implemento', ['$scope','$http','$mdToast', function($scope, $http,$mdToast){
	$scope.obj={};
	$scope.obj2={};
	$scope.tcondicion=[];	
	...
	$scope.cargarTipos=function(){
		return $http.get('Admin/tipoImplemento/ver_sel').
	...
	$scope.crear_nuevo= function(tipo){
		...
		if(tipo){
			url='Admin/implemento/crear_implemento';
			obj=$scope.obj;
		}else{
			url='Admin/implemento/modificar_implemento';
			obj=$scope.obj2;
		}
	...
	$scope.get= function(id){	
		$scope.cargarTipos();
		$http.get('Admin/implemento/ver/'+id).
		success(function(data, status, headers, config) {				
			data.estatus=data.estatus=='1';			
			data.defecto=data.defecto=='1';			
			console.log(data);
			$scope.obj2=data;										
	...
	$scope.getResource = function (params, paramsObj) {	
		...
		var urlApi = 'Admin/implemento/tabla_principal_implemento/'+paramsObj.count+'/'+paramsObj.page+'/';
		if(paramsObj.sortBy){
